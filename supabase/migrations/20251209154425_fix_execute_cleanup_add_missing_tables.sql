/*
  ...
  ## 问题
  - valid_order_data 表在配置中但没有清理实现
  
  ## 解决方案
  - 添加 valid_order_data 的清理逻辑
  - 确保所有配置的表都有对应的清理实现
*/
...
BEGIN
  ...
  IF p_admin_id IS NOT NULL THEN
    INSERT INTO history_cleanup_log (
      ...
      p_days_to_keep,
      'completed',
      'Deleted ' || v_deleted || ' records (' || ROUND(v_delete_percentage, 1) || '%). Run VACUUM to reclaim disk space.'
    );
  END IF;
  ...
  RETURN QUERY SELECT
    ...
    ('Successfully deleted ' || v_deleted || ' records (' || ROUND(v_delete_percentage, 1) || '%). Space will be reclaimed after next VACUUM.')::text;
END;
$$;

COMMENT ON FUNCTION execute_cleanup IS 'Execute cleanup for all configured tables without VACUUM';
