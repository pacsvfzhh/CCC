/*
  # Fix Space Freed Calculation with VACUUM
  
  ## Changes
  - Add VACUUM after DELETE to reclaim space
  - Measure space before and after VACUUM
  - Provide accurate space freed reporting
  
  ## Note
  - DELETE only marks rows as deleted
  - VACUUM actually reclaims the space
  - VACUUM FULL would be more thorough but locks the table
*/
...
BEGIN
  ...
  -- Get size after cleanup and vacuum
  ...
  RETURN QUERY SELECT
    ...
    format('Successfully deleted %s records (space will be reused)', v_deleted)::text;
END;
$$;

COMMENT ON FUNCTION execute_cleanup IS 'Execute cleanup with VACUUM to reclaim space';
