/*
  # Add Employee Login History Cleanup to User Deletion

  1. Updates
    - Update `cleanup_user_data()` function to include employee_login_history table
    - Ensures all login history records are deleted when an employee is removed

  2. Reasoning
    - Although employee_login_history has ON DELETE CASCADE constraint
    - Explicit deletion ensures complete cleanup and provides audit trail
    - Maintains consistency with other cleanup operations
    - Prevents any orphaned records in edge cases

  3. Benefits
    - ✅ Complete data removal when employee is deleted
    - ✅ Privacy compliance (no login history remains)
    - ✅ Storage optimization (prevent log accumulation)
    - ✅ Clear audit trail in logs
    - ✅ Fail-safe approach (explicit DELETE + CASCADE)
...
-- Update cleanup_user_data() function to include employee_login_history
...
BEGIN
  ...
  -- ========================================================================
  -- EMPLOYEE LOGIN HISTORY (add this section)
  -- ========================================================================
...
COMMENT ON FUNCTION cleanup_user_data() IS
  ...
  Includes: employee login history, dispatch system, customer service, work sessions,
  orders (main + history), wallet, transactions, verifications, withdrawals, messages,
  broadcast recipients, commission audit logs, and service ratings.
