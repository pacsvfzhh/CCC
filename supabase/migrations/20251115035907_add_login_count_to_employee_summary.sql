/*
  # Add Login Count to Employee Login Summary

  1. Changes
    - Drop and recreate `get_employee_login_summary` function to include `total_logins` count
    - This provides more relevant information for the login history panel

  2. Details
    - Count all 'login' action types for each employee
    - Return as `total_logins` field in the result
...
-- Recreate function with login count
